...
Este relatório documenta a entrega de um pipeline local de RLHF inspirado no formato textual do InstructGPT. O trabalho parte do dataset \texttt{databricks/databricks-dolly-15k} e constrói quatro etapas: preparação dos datasets de SFT, RM e PPO; fine-tuning supervisionado do GPT-2; treinamento de um reward model baseado em GPT-2; e ajuste final do modelo SFT com PPO usando o reward model treinado.
...
Quando não existe contexto, o prompt contém apenas a instrução original. Essa decisão segue o espírito do InstructGPT: entradas textuais diretas, demonstrações humanas para SFT, preferências para RM e prompts sem resposta para PPO.

\section{Artefatos Entregues}

Todos os artefatos foram organizados dentro de \texttt{rlhf-project/}. Os principais scripts gerados foram:
...
A primeira tarefa consistiu em transformar o Dolly 15k em artefatos compatíveis com as etapas clássicas de RLHF. Foram criados quatro \texttt{DatasetDicts} locais, todos com splits \texttt{train}, \texttt{validation} e \texttt{test}, estratificados por categoria com seed 42.
...
Na segunda etapa, o modelo \texttt{gpt2} base foi ajustado com SFT usando o dataset \texttt{rlhf\_dolly\_datasets/sft}. O treinamento utilizou três épocas, avaliação por época, seleção do melhor checkpoint por \texttt{eval\_loss}, \texttt{max\_length=1024}, batch por GPU 8, gradient accumulation 4, learning rate $5 \times 10^{-5}$, warmup de 3\%, weight decay 0.01 e treinamento distribuído em quatro GPUs.
...
A terceira etapa treinou um reward model baseado em \texttt{GPT2ForSequenceClassification}, inicializado a partir do checkpoint SFT em \texttt{sft\_gpt2/model}. O treinamento usou o dataset \texttt{rm\_synthetic}, no qual a resposta Dolly é \texttt{chosen} e a resposta gerada pelo SFT é \texttt{rejected}. A função objetivo foi a loss pairwise Bradley-Terry:
...
A quarta etapa fechou o pipeline RLHF com PPO. A política inicial foi \texttt{sft\_gpt2/model}, a referência KL congelada também foi \texttt{sft\_gpt2/model}, e o reward model congelado foi \texttt{reward\_gpt2/model}. O dataset usado foi o \texttt{rlhf\_dolly\_datasets/ppo}, que contém apenas prompts.
