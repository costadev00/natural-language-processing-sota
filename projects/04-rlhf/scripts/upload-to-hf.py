...
def get_hf_token(explicit_env_file: str | None) -> str:
    ...
    candidate_paths.extend([Path(".env"), Path("..") / ".env", PROJECT_DIR / ".env", REPO_ROOT / ".env"])
...
def build_staging_dir(dataset_dir: Path, staging_dir: Path, repo_id: str) -> None:
    ...
    shutil.copy2(SCRIPT_DIR / "build-dolly-rlhf-datasets.py", staging_dir / "build-dolly-rlhf-datasets.py")
    shutil.copy2(PROJECT_DIR / "requirements.txt", staging_dir / "requirements.txt")
